...
$titulo = 'REPORTE DE EXISTENCIAS POR ARTICULO - AGRUPADO POR SEDE-AREA';
...
try {
    ...
    $sql = "SELECT AR.id_area,AR.nom_area,SUM(HV.valor) AS val_total
            FROM acf_hojavida AS HV
            INNER JOIN far_medicamentos AS ME ON (ME.id_med=HV.id_articulo)
            INNER JOIN far_subgrupos AS FG ON (FG.id_subgrupo=ME.id_subgrupo)
            INNER JOIN far_centrocosto_area AS AR ON (AR.id_area=HV.id_area)
            $where AND HV.id_sede=:id_sede 
            GROUP BY AR.id_area
            ORDER BY AR.nom_area";
    $rs_a = $cmd->prepare($sql);
    ...
    $sql = "SELECT ME.id_med,ME.cod_medicamento,ME.nom_medicamento,ME.top_min,ME.top_max,
            ...
            INNER JOIN far_medicamentos AS ME ON (ME.id_med=HV.id_articulo)
            INNER JOIN far_subgrupos AS FG ON (FG.id_subgrupo=ME.id_subgrupo)
            $where AND HV.id_sede=:id_sede AND HV.id_area=:id_area
            GROUP BY ME.id_med
    ...
    <table style="width:100% !important">
        <tbody style="font-size: 60%;">
            ...
            foreach ($objs as $obj1) {
                ...
                $rs_a->bindParam(':id_sede', $id_sede);
                $rs_a->execute();
                $obja = $rs_a->fetchAll();

                foreach ($obja as $obj2) {
                    $id_area = $obj2['id_area'];
                    $tabla .= '<tr><th colspan="8" style="text-align:left">' . str_repeat('&nbsp', 10) . strtoupper($obj2['nom_area']) . '</th><th style="text-align:right">' . formato_valor($obj2['val_total']) . '</th></tr>';
