...
if (!empty($objs)) {
    foreach ($objs as $obj) {
        ...
        if ($permisos->PermisosUsuario($opciones, 5401, 3) || $id_rol == 1) {
            $anular = '<a value="' . $obj['id_pto_crp_det'] . '" class="btn btn-outline-danger btn-xs rounded-circle shadow me-1 btn_anular_liberacion_crp" title="Anular liberacion"><span class="fas fa-trash-restore"></span></a>';
            $imprimir = '<a value="' . $obj['id_pto_crp_det'] . '" class="btn btn-outline-success btn-xs rounded-circle shadow me-1 btn_imprimir_liberacion_crp" title="Imprimir liberacion"><span class="fas fa-print"></span></a>';
        }
