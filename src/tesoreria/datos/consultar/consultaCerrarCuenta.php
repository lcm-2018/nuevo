...
try {
    ...
    $query = $cmd->prepare("UPDATE tes_cuentas SET estado=1 WHERE id_tes_cuenta=?");
    $query->bindParam(1, $data, PDO::PARAM_INT);
    $query->execute();
    if ($query->rowCount() > 0) {
        Logs::guardaLog("UPDATE tes_cuentas SET estado=1 WHERE id_tes_cuenta=$data");
    }
    ...
    $query->execute();
    if ($query->rowCount() > 0) {
        Logs::guardaLog("UPDATE ctb_pgcp SET estado=1 WHERE id_pgcp=$id_pgcp");
    }
