...
try {
    ...
    $sql = "SELECT
                ...
                , ctb_doc.detalle
                , SUM(IFNULL(ctb_libaux.debito,0)) AS causacion 
                , SUM(IFNULL(retencion.vr_retencion,0)) AS retencion
                , (SUM(IFNULL(ctb_libaux.debito,0)) - SUM(IFNULL(retencion.vr_retencion,0))) AS neto
            FROM
                ctb_libaux
                INNER JOIN ctb_doc ON (ctb_libaux.id_ctb_doc = ctb_doc.id_ctb_doc)    
                INNER JOIN tb_terceros ON (tb_terceros.id_tercero_api = ctb_libaux.id_tercero_api)
                LEFT JOIN (SELECT id_ctb_doc,SUM(valor_retencion) AS vr_retencion FROM ctb_causa_retencion GROUP BY id_ctb_doc) AS retencion     
                        ON (retencion.id_ctb_doc = ctb_doc.id_ctb_doc)
                WHERE ctb_doc.id_tipo_doc = 3
                AND ctb_libaux.debito > 0
                AND ctb_doc.fecha BETWEEN $fec_ini AND $fec_fin
