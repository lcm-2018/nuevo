    ...
    $sql = "SELECT 
            ...
            FROM
                ...
                WHERE `ctb_doc`.`estado` = 2
                    AND ((SUBSTRING(`ctb_pgcp`.`cuenta`, 1, 4) IN ('1105') AND `ctb_doc`.`fecha` < '$fecha_inicial'))
                ...
                FROM
                    ...
                    INNER JOIN `ctb_pgcp` 
                        ON (`ctb_libaux`.`id_cuenta` = `ctb_pgcp`.`id_pgcp`)
                WHERE (SUBSTRING(`ctb_pgcp`.`cuenta`, 1, 4) IN ('1105') AND `ctb_doc`.`fecha` BETWEEN '$fecha_inicial' AND '$fecha_corte' AND `ctb_doc`.`estado` = 2)
...
foreach ($datos as $dato) {
    ...
    foreach ($cuentas as $c) {
        ...
        if (($c['tipo_dato'] == 'M' && strpos($cuenta, $c['cuenta']) === 0) || ($c['tipo_dato'] != 'M' && $cuenta == $c['cuenta'])) {
            ...
            if ($_POST['xtercero'] == 1 && (substr($c['cuenta'], 0, 2) === "11") && $c['tipo_dato'] == 'D') {
