...
try {
    ...
    if (!($sql->execute())) {
        ...
        if ($sql->rowCount() > 0) {
            Logs::guardaLog("UPDATE `pto_cdp` SET `estado` = $estado WHERE `id_pto_cdp` = $id_cdp");
            ...
            $sql->execute();
            if ($sql->rowCount() > 0) {
                Logs::guardaLog("UPDATE `pto_cdp` SET `id_user_act` = $id_user, `fecha_act` = '$fecha' WHERE `id_pto_cdp` = $id_cdp");
            }
            $response['status'] = 'ok';
        } else {
            $response['status'] = 'ok';
