...
try {
    ...
    if (!($sql->execute())) {
        ...
        if ($sql->rowCount() > 0) {
            Logs::guardaLog("UPDATE `pto_rad` SET `estado` = $estado WHERE `id_pto_rad` = $id_rad");
            ...
            $sql->execute();
            if ($sql->rowCount() > 0) {
                Logs::guardaLog("UPDATE `pto_rad` SET `id_user_act` = $id_user, `fecha_act` = '$fecha' WHERE `id_pto_rad` = $id_rad");
            }
            $response['status'] = 'ok';
        } else {
            $response['status'] = 'ok';
