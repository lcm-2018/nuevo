...
if (isset($_POST)) {
    ...
    if (empty($_POST['id_pto_mvto'])) {
        ...
        $query->execute();
        if ($cmd->lastInsertId() > 0) Logs::guardaLog("INSERT INTO pto_documento_detalles (id_pto_doc, tipo_mov, rubro, valor,estado) VALUES ($id_pto_cdp, '$tipo_mov', '$rubro', $valorCdp, 0)");
        $cmd = null;
        echo "ok";
    } else {
        ...
        $query->execute();
        if ($query->rowCount() > 0) Logs::guardaLog("UPDATE pto_documento_detalles SET id_pto_doc = $id_pto_cdp, tipo_mov = '$tipo_mov', rubro ='$rubro', valor = $valorCdp WHERE id_pto_mvto = $id");
        $cmd = null;
        echo "modificado";
    }
}
