...
if (!empty($retenciones)) {
    foreach ($retenciones as $lp) {
        ...
        if ($permisos->PermisosUsuario($opciones, 5506, 4) || $id_rol == 1) {
            $borrar = '<a text="' . $id_r . '" href="javascript:void(0)" class="btn btn-outline-danger btn-xs rounded-circle me-1 shadow borrar"  title="Eliminar"><span class="fas fa-trash-alt"></span></a>';
        }
