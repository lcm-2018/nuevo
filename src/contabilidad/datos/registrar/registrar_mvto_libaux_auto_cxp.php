...
try {
    ...
    $query = "SELECT
                `cd`.`id_ctb_doc`,
                `cla_d`.`id_cuenta` AS `cuenta_debito`,
                `cla_c`.`id_cuenta` AS `cuenta_credito`
            FROM `ctb_doc` AS `cd`
            INNER JOIN `ctb_libaux` AS `cla_d`
                ON `cla_d`.`id_ctb_doc` = `cd`.`id_ctb_doc`
                AND `cla_d`.`debito` > 0
                AND `cla_d`.`id_tercero_api` = $id_tercero
            INNER JOIN `ctb_libaux` AS `cla_c`
                ON `cla_c`.`id_ctb_doc` = `cd`.`id_ctb_doc`
                AND `cla_c`.`credito` > 0
                AND `cla_c`.`ref` = 1
                AND `cla_c`.`id_tercero_api` = $id_tercero
            WHERE `cd`.`estado` > 0
            ORDER BY `cd`.`id_ctb_doc` DESC
            LIMIT 1";
    $rs = $cmd->query($query);
    $cuentas = $rs->fetch();
    $rs->closeCursor();
    ...
    if (empty($ingresos)) {
        foreach ($datoscostos as $dc) {
            $id_cuenta = $cuentas['cuenta_debito'] ?? NULL;
            $debito = $dc['valor'];
            $query->execute();
    ...
    if (empty($ingresos)) {
        foreach ($datoscostos as $dc) {
            $id_cuenta = $cuentas['cuenta_credito'] ?? NULL;
