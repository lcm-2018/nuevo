...
$tipo = isset($_POST['id']) ? (int) $_POST['id'] : 0;
...
try {
    ...
    echo $e->getCode() == 2002 ? 'Sin Conexion a Mysql (Error: 2002)' : 'Error: ' . $e->getCode();
    exit();
}
?>
<div class="px-0">
    <div class="shadow">
        <div class="card-header text-center py-2" style="background-color: #16a085 !important;">
            <h5 class="mb-0 text-white">CONSECUTIVOS DISPONIBLES</h5>
        </div>
        <div class="p-3">
            <?php if (!empty($consecutivos)): ?>
                <div class="row g-2">
                    <?php foreach ($consecutivos as $consecutivo): ?>
                        <div class="col-auto">
                            <span class="badge bg-success fs-6 px-3 py-2"><?= $consecutivo['consecutivo']; ?></span>
                        </div>
                    <?php endforeach; ?>
                </div>
            <?php else: ?>
