...
foreach ($retenciones as $ret) {
    ...
    $response .= '<div class="form-control form-control-sm bg-input text-start">' . $ret['nom_municipio'] . '->' . pesos($base) . '</div>';
    $response .= '</div>';
    $response .= '<div class="col-md-2">';
    if ($primero) {
        $response .= $lbl_dat;
    }
    $response .= '<select class="form-control form-control-sm bg-input py-0 sm" name="id_rete[]" onchange="aplicaDctoRetIca(this,value,1)" required>';
    ...
    $response .= '<select class="form-control form-control-sm bg-input py-0 sm" name="id_rete_sobre[]" onchange="aplicaDctoRetIca(this,value,2)" required>';
    $response .= '<option value="0">-- Seleccionar--</option>';
