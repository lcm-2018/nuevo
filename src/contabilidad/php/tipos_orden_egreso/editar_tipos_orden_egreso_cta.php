...
try {
    ...
    if (($permisos->PermisosUsuario($opciones, 5511, 2) && $oper == 'add' && $_POST['id_tipo_egreso_cta'] == -1) ||
        ($permisos->PermisosUsuario($opciones, 5511, 3) && $oper == 'add' && $_POST['id_tipo_egreso_cta'] != -1) ||
        ($permisos->PermisosUsuario($opciones, 5511, 4) && $oper == 'del') || $id_rol == 1
    ) {
            ...
            if ($oper == 'add') {
                ...
                if ($obj_tegr['consumo'] == 0) {
                    ...
                    if ($id == -1) {
                        ...
                        if ($rs) {
                            $res['mensaje'] = 'ok';
                            Logs::guardaLog("INSERT INTO far_orden_egreso_tipo_cta(id_tipo_egreso,id_cuenta,fecha_vigencia,estado,id_usr_crea) VALUES($id_tipo_egreso,$id_cta,$fec_vig,$estado,$id_usr_crea)");
                        ...
                        if ($rs) {
                            $res['mensaje'] = 'ok';
                            Logs::guardaLog("UPDATE far_orden_egreso_tipo_cta SET id_cuenta=$id_cta,fecha_vigencia=$fec_vig,estado=$estado WHERE id_tipo_egreso_cta=$id");
                            $res['id'] = $id;
                        } else {
                            $res['mensaje'] = $cmd->errorInfo()[2];
                        }
