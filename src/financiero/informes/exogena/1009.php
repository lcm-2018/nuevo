...
try {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $linea = [
            ...
            $es_cc ? ($nombre['apellido1'] ?? '') : '',         // Primer apellido
            $es_cc ? ($nombre['apellido2'] ?? '') : '',         // Segundo apellido
            $es_cc ? ($nombre['nombre1']   ?? '') : '',         // Primer nombre
            $es_cc ? ($nombre['nombre2']   ?? '') : '',         // Otros nombres
            !$es_cc ? $row['nom_tercero']  : '',                // Razón social
