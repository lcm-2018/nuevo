...
// Obtener datos de libranzas agrupadas por entidad
$libranzasObj = new Libranzas();
$datosLibranzas = $libranzasObj->getLibranzasPorNomina($id_nomina);

if (empty($datosLibranzas)) {
    exit('No se encontraron libranzas para esta nómina');
}

// Si es tipo Excel, generar archivo Excel
if ($tipo == 'E') {
    header('Content-Type: application/vnd.ms-excel; charset=UTF-8');
    header('Content-Disposition: attachment; filename="Libranzas_Nomina_' . $id_nomina . '_' . date('Y-m-d') . '.xls"');
    ...
    echo "<tr><td colspan='3' style='text-align: center; font-weight: bold;'>REPORTE DE LIBRANZAS</td></tr>";
    echo "<tr><td colspan='3' style='text-align: center;'>NÓMINA No. {$id_nomina} - MES: {$mes} - VIGENCIA: {$nomina['vigencia']}</td></tr>";
    echo "<tr><td colspan='3'>&nbsp;</td></tr>";

    $gran_total = 0;

    foreach ($datosLibranzas as $entidad) {
...
$documento = "Reporte de Libranzas";
