...
// Obtener datos de embargos agrupadas por juzgado
$embargosObj = new Embargos();
$datosEmbargos = $embargosObj->getEmbargosPorNomina($id_nomina);

if (empty($datosEmbargos)) {
    exit('No se encontraron embargos para esta nómina');
}

// Si es tipo Excel, generar archivo Excel
if ($tipo == 'E') {
    header('Content-Type: application/vnd.ms-excel; charset=UTF-8');
    header('Content-Disposition: attachment; filename="Embargos_Nomina_' . $id_nomina . '_' . date('Y-m-d') . '.xls"');
    ...
    // Encabezado
    echo "<table border='1'>";
    echo "<tr><td colspan='4' style='text-align: center; font-weight: bold; font-size: 14px;'>" . mb_strtoupper($empresa['nombre']) . "</td></tr>";
    echo "<tr><td colspan='4' style='text-align: center; font-weight: bold;'>REPORTE DE EMBARGOS</td></tr>";
    echo "<tr><td colspan='4' style='text-align: center;'>NÓMINA No. {$id_nomina} - MES: {$mes} - VIGENCIA: {$nomina['vigencia']}</td></tr>";
    echo "<tr><td colspan='4'>&nbsp;</td></tr>";

    $gran_total = 0;

    foreach ($datosEmbargos as $entidad) {
        // Encabezado de entidad
        echo "<tr style='background-color: #f0f0f0;'>";
        echo "<td colspan='4' style='font-weight: bold;'>{$entidad['nit_entidad']} {$entidad['nom_entidad']}</td>";
        echo "</tr>";
        ...
        foreach ($entidad['empleados'] as $empleado) {
            ...
            echo "<td>{$empleado['tipo_embargo']}</td>";
    ...
    echo "<td colspan='3' style='text-align: right;'>GRAN TOTAL:</td>";
...
foreach ($datosEmbargos as $entidad) {
    ...
    $html .= "<th colspan='4' style='text-align: left; padding: 8px; font-size: 12px; font-weight: bold;'>";
    ...
    $html .= "<th style='padding: 6px; text-align: left; font-size: 11px;'>Tipo Embargo</th>";
    ...
    foreach ($entidad['empleados'] as $empleado) {
        ...
        $html .= "<td style='padding: 4px 8px;'>{$empleado['tipo_embargo']}</td>";
    ...
    $html .= "<td colspan='3' style='padding: 6px 8px; text-align: right;'>TOTAL:</td>";
