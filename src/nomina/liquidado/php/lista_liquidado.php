...
if (!empty($obj)) {
    foreach ($obj as $o) {
        ...
        if (($permisos->PermisosUsuario($opciones, 5101, 6) || $id_rol == 1)) {
            $imprimir3 = '<button data-id="' . $id . '" text="N" class="btn btn-outline-primary btn-xs rounded-circle shadow me-1 imprimir" title="Imprimir Nómina"><span class="fas fa-print fa-sm"></span></button>';
        }

        if (($permisos->PermisosUsuario($opciones, 5115, 1) || $id_rol == 1)) {
            $reportes = '<button data-id="' . $id . '" class="btn btn-outline-dark btn-xs rounded-circle shadow me-1 reportes" title="Reportes"><span class="fas fa-chart-bar fa-sm"></span></button>';
        }
