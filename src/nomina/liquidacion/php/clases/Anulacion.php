...
class Anulacion
{
    ...
    public function anulaRegistros($id_empleado, $id_nomina, $tipo = 1)
    {
        ...
        // Determinar si se filtra por empleado o se aplica a todos
        $filtroEmpleado = $id_empleado > 0;
        $condEmpleado = $filtroEmpleado ? "`id_empleado` = :id_empleado AND" : "";
        $condEmpleadoAlias = function ($alias) use ($filtroEmpleado) {
        ...
        $queries = [
            ...
            // compesatorio
            "UPDATE `nom_liq_compesatorio` SET `estado` = 0 WHERE {$condEmpleado} `id_nomina` = :id_nomina",
            ...
            "UPDATE `nom_liq_embargo` AS `nle`
             ...
             SET `nle`.`estado` = 0
             WHERE {$condEmpleadoAlias('ne')} `nle`.`id_nomina` = :id_nomina",

            //"UPDATE `nom_liq_empleado` SET `estado` = 0 WHERE {$condEmpleado} `id_nomina` = :id_nomina",

            // incapacidades
            "UPDATE `nom_liq_incap` AS `nli`
                INNER JOIN `nom_incapacidad` AS `ni` ON `nli`.`id_incapacidad` = `ni`.`id_incapacidad`
             SET `nli`.`estado` = 0
             WHERE {$condEmpleadoAlias('ni')} `nli`.`id_nomina` = :id_nomina",
            ...
            "UPDATE `nom_liq_sindicato_aportes` AS `nlsa`
             ...
             SET `nlsa`.`estado` = 0
             WHERE {$condEmpleadoAlias('ncs')} `nlsa`.`id_nomina` = :id_nomina",

            // vacaciones
            "UPDATE `nom_liq_vac` AS `nlv`
                INNER JOIN `nom_vacaciones` AS `nv` ON `nlv`.`id_vac` = `nv`.`id_vac`
             SET `nlv`.`estado` = 0
             WHERE {$condEmpleadoAlias('nv')} `nlv`.`id_nomina` = :id_nomina AND `nlv`.`tipo`  = 'S'",

            // parafiscales y prestaciones
            "UPDATE `nom_liq_parafiscales` SET `estado` = 0 WHERE {$condEmpleado} `id_nomina` = :id_nomina",
            "UPDATE `nom_liq_prestaciones_sociales` SET `estado` = 0 WHERE {$condEmpleado} `id_nomina` = :id_nomina",

            // primas
            "UPDATE `nom_liq_prima` SET `estado` = 0 WHERE {$condEmpleado} `id_nomina` = :id_nomina $tipo",
            "UPDATE `nom_liq_prima_nav` SET `estado` = 0 WHERE {$condEmpleado} `id_nomina` = :id_nomina $tipo",
