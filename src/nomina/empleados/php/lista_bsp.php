...
if (!empty($obj)) {
    foreach ($obj as $o) {
        ...
        if ($o['tipo'] == 'S' || $o['tipo'] == 'P' || $o['estado'] == 0) {
            $tipo = '<span class="badge bg-primary">SISTEMA</span>';
            $actualizar = $eliminar = '';
        }
        $tipo = $tipo ?? '<span class="badge bg-info">MANUAL</span>';

        $datos[] = [
            'id'        => $id,
            'corte'     => $o['fec_corte'],
            'valor'     => $o['val_bsp'],
            'tipo'      => '<div class="text-center">' . $tipo . '</div>',
