...
class Otros_Descuentos
{
    ...
    public function addRegistro($array)
    {
        try {
            ...
            if ($id > 0) {
                ...
                Logs::guardaLog("INSERT INTO `nom_otros_descuentos` (`id_empleado`,`id_tipo_dcto`,`fecha`,`fecha_fin`,`concepto`,`valor`,`estado`,`id_user_reg`,`fec_reg`) VALUES ({$array['id_empleado']}, {$array['slcTipoDcto']}, '{$array['datFecInicia']}', $fecFin, '{$array['txtDescribe']}', {$array['numValor']}, 1, $idUser, '$hoy')");
                return 'si';
            } else {
    ...
    public function addRegistroLiq($array)
    {
        try {
            ...
            if ($id > 0) {
                $hoy = Sesion::Hoy();
                $idUser = Sesion::IdUser();
                Logs::guardaLog("INSERT INTO `nom_liq_descuento` (`id_dcto`,`valor`,`id_nomina`,`id_user_reg`,`fec_reg`) VALUES ({$array['id_dcto']}, {$array['valor']}, {$array['id_nomina']}, $idUser, '$hoy')");
                return 'si';
            } else {
                return 'No se insertó el registro';
            }
    ...
    public function editRegistroLiq($array)
    {
        try {
            ...
            if ($stmt->execute() && $stmt->rowCount() > 0) {
                Logs::guardaLog("UPDATE `nom_liq_descuento` SET `valor` = {$array['valor_otros_descuentos']} WHERE `id_liq` = {$array['id']}");
                ...
                $stmt2->execute();
                Logs::guardaLog("UPDATE `nom_liq_descuento` SET `fec_act` = '$hoy', `id_user_act` = $idUser WHERE `id_liq` = {$array['id']}");
                return 'si';
            } else {
                return 'no';
            }
    ...
    public function editRegistro($array)
    {
        try {
            ...
            if ($stmt->execute() && $stmt->rowCount() > 0) {
                $fecFin = $array['datFecFin'] == '' ? 'NULL' : "'{$array['datFecFin']}'";
                Logs::guardaLog("UPDATE `nom_otros_descuentos` SET `id_tipo_dcto` = {$array['slcTipoDcto']}, `fecha` = '{$array['datFecInicia']}', `fecha_fin` = $fecFin, `concepto` = '{$array['txtDescribe']}', `valor` = {$array['numValor']} WHERE `id_dcto` = {$array['id']}");
                ...
                $stmt2->bindValue(3, $array['id'], PDO::PARAM_INT);
                $stmt2->execute();
                Logs::guardaLog("UPDATE `nom_otros_descuentos` SET `fec_act` = '$hoy', `id_user_act` = $idUser WHERE `id_dcto` = {$array['id']}");
                return 'si';
            } else {
    ...
    public function annulRegistro($array)
    {
        try {
            ...
            $stmt->bindValue(2, $array['id'], PDO::PARAM_INT);
            if ($stmt->execute() && $stmt->rowCount() > 0) {
                Logs::guardaLog("UPDATE `nom_otros_descuentos` SET `estado` = {$array['estado']} WHERE `id_dcto` = {$array['id']}");
                return 'si';
            } else {
