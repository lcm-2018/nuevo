...
/**
 * Clase para gestionar las licencias de luto de los empleados.
 * 
 * Esta clase permite realizar operaciones CRUD sobre las licencias de luto de los empleados,
 * incluyendo la obtención de registros, adición, edición y eliminación de licencias.
 */
class Licencias_Luto
{
    private $conexion;
            ...
            <<<HTML
                <div class="shadow text-center rounded">
                    <div class="rounded-top py-2" style="background-color: #16a085 !important;">
                        <h5 style="color: white;" class="mb-0">GESTIÓN DE LICENCIA POR LUTO</h5>
                    </div>
                    <div class="p-3">
                        <form id="formLicenciasLuto">
                    ...
                    <div class="text-center pb-3">
                        <button type="button" class="btn btn-primary btn-sm" id="btnGuardarLicenciaLuto">Guardar</button>
    ...
    {
        try {
            $sql = "DELETE FROM `nom_licencia_luto` WHERE `id_licluto` = ?";
            $consulta  = "DELETE FROM `nom_licencia_luto` WHERE `id_licluto` = $id";
            $stmt = $this->conexion->prepare($sql);
            $stmt->bindParam(1, $id, PDO::PARAM_INT);
            $stmt->execute();
            if ($stmt->rowCount() > 0) {
    ...
    public function addRegistro($array)
    {
        try {
            ...
            if ($id > 0) {
                ...
                Logs::guardaLog("INSERT INTO `nom_licencia_luto` (`id_empleado`,`fec_inicio`,`fec_fin`,`dias_inactivo`,`dias_habiles`,`fec_reg`,`id_user_reg`) VALUES ({$array['id_empleado']}, '{$array['datFecInicia']}', '{$array['datFecFin']}', {$array['diasInactivo']}, {$array['diasHabiles']}, '$hoy', $idUser)");
                $array['novedad'] = $id;
                $array['tipo'] = 5;
    ...
    public function addRegistroLiq($array)
    {
        try {
            ...
            if ($id > 0) {
                $hoy = Sesion::Hoy();
                $idUser = Sesion::IdUser();
                Logs::guardaLog("INSERT INTO `nom_liq_licluto` (`id_licluto`,`dias_licluto`,`val_liq`,`id_user_reg`,`fec_reg`,`id_nomina`) VALUES ({$array['id_licluto']}, {$array['dias']}, {$array['valor']}, $idUser, '$hoy', {$array['id_nomina']})");
                return 'si';
            } else {
                return 'No se insertó el registro';
            }
    ...
    public function editRegistro($array)
    {
        try {
            ...
            if ($stmt->execute() && $stmt->rowCount() > 0) {
                Logs::guardaLog("UPDATE `nom_licencia_luto` SET `fec_inicio` = '{$array['datFecInicia']}', `fec_fin` = '{$array['datFecFin']}', `dias_inactivo` = {$array['diasInactivo']}, `dias_habiles` = {$array['diasHabiles']} WHERE `id_licluto` = {$array['id']}");
                $consulta = "UPDATE `nom_licencia_luto` SET `fec_act` = ? WHERE `id_licluto` = ?";
                $stmt2 = $this->conexion->prepare($consulta);
                $hoy = Sesion::Hoy();
                $stmt2->bindValue(1, $hoy, PDO::PARAM_STR);
                $stmt2->bindValue(2, $array['id'], PDO::PARAM_INT);
                $stmt2->execute();
                Logs::guardaLog("UPDATE `nom_licencia_luto` SET `fec_act` = '$hoy' WHERE `id_licluto` = {$array['id']}");
                $Novedad = new Novedades($this->conexion);
