...
switch ($id_reporte) {
    case '6':
        $titulo = 'REPORTE DE EXISTENCIAS TOTALIZADO POR ARTICULO - AGRUPADO POR SEDE-BODEGA-SUBGRUPO';
        break;
}
...
try {
    ...
    $sql = "SELECT far_medicamentos.cod_medicamento,far_medicamentos.nom_medicamento,
                ...
                SUM(far_medicamento_lote.existencia*far_medicamentos.val_promedio) AS val_total
            FROM far_medicamento_lote
            INNER JOIN far_medicamentos ON (far_medicamentos.id_med=far_medicamento_lote.id_med)
            INNER JOIN far_subgrupos ON (far_subgrupos.id_subgrupo=far_medicamentos.id_subgrupo)
            INNER JOIN far_bodegas ON (far_bodegas.id_bodega = far_medicamento_lote.id_bodega)
    ...
    <table style="width:100% !important">
        <tbody style="font-size: 60%;">
            ...
            switch ($id_reporte) {
                case '6':
                    $tabla = '<tr style="background-color:#CED3D3; text-align:center">
                        <th>Código Art.</th><th>Nombre</th><th>Existencia</th><th>Vr. Promedio</th><th>Vr. Parcial</th><th>Vr. Total</th></tr>';
                    break;
            }
            ...
            foreach ($objs as $obj1) {
                ...
                $tabla .= '<tr><th colspan="5" style="text-align:left">' . strtoupper($obj1['nom_sede'] . ' - ' . $obj1['nom_bodega']) . '</th><th style="text-align:right">' . formato_valor($obj1['val_total_sb']) . '</th></tr>';
                ...
                foreach ($objg as $obj2) {
                    ...
                    $tabla .= '<tr><th colspan="4" style="text-align:left">' . str_repeat('&nbsp', 10) . strtoupper($obj2['nom_subgrupo']) . '</th><th style="text-align:right">' . formato_valor($obj2['val_total_sg']) . '</th></tr>';
