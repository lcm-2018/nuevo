...
try {
    ...
    if (($permisos->PermisosUsuario($opciones, 5005, 2) && $oper == 'add' && $_POST['id_pedido'] == -1) ||
        ($permisos->PermisosUsuario($opciones, 5005, 3) && $oper == 'add' && $_POST['id_pedido'] != -1) ||
        ($permisos->PermisosUsuario($opciones, 5005, 4) && $oper == 'del') ||
        ($permisos->PermisosUsuario($opciones, 5005, 3) && $oper == 'conf') ||
        ($permisos->PermisosUsuario($opciones, 5005, 3) && $oper == 'close') ||
        ($permisos->PermisosUsuario($opciones, 5005, 5) && $oper == 'annul' || $id_rol == 1)
    ) {
        ...
        if ($oper == 'add') {
            ...
            if ($id == -1) {
                ...
                if ($rs) {
                    ...
                    $proceso = "Se Registró Pedido de Almacén Id: " . $obj['id'] . ", Fecha: " . $fec_ped . ", Detalle: " . $detalle;
                ...
                if ($obj_pedido['estado'] == 1) {
                    ...
                    if ($rs) {
                        ...
                        $proceso = "Se Modificó Pedido de Almacén Id: " . $id . ", Fecha: " . $fec_ped . ", Detalle: " . $detalle;
                        Logs::guardaLog($proceso);
                    } else {
                        $res['mensaje'] = $cmd->errorInfo()[2];
                    }
        ...
        if ($oper == 'del') {
            ...
            if ($obj_pedido['estado'] == 1) {
                ...
                if ($rs) {
                    ...
                    $proceso = "Se Eliminó Pedido de Almacén Id: " . $id . ", Fecha: " . $obj_pedido['fec_pedido'] . ", Detalle: " . $obj_pedido['detalle'];
                    Logs::guardaLog($proceso);
                } else {
                    $res['mensaje'] = $cmd->errorInfo()[2];
                }
        ...
        if ($oper == 'conf') {
            ...
            if ($estado == 1 && $num_detalles > 0) {
                ...
                if ($error == 0) {
                    ...
                    $proceso = "Se Confirmó Pedido de Almacén Id: " . $id . ", Fecha: " . $obj_pedido['fec_pedido'] . ", Detalle: " . $obj_pedido['detalle'];
                    Logs::guardaLog($proceso);
                } else {
        ...
        if ($oper == 'annul') {
            ...
            if ($obj_pedido['estado'] == 2) {
                ...
                if ($rs) {
                    ...
                    $proceso = "Se Anuló Pedido de Almacén Id: " . $id . ", Fecha: " . $obj_pedido['fec_pedido'] . ", Detalle: " . $obj_pedido['detalle'];
                    Logs::guardaLog($proceso);
                } else {
                    $error = $cmd->errorInfo();
        ...
        if ($oper == 'close') {
            ...
            if ($obj_pedido['estado'] == 3) {
                ...
                if ($rs) {
                    ...
                    $proceso = "Se Cerró Pedido de Almacén Id: " . $id . ", Fecha: " . $obj_pedido['fec_pedido'] . ", Detalle: " . $obj_pedido['detalle'];
                    Logs::guardaLog($proceso);
                } else {
                    $error = $cmd->errorInfo();
                    $res['mensaje'] = 'Error en base de datos-far_alm_pedido:' . $error[2];
